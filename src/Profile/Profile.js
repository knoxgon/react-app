...
export default class Profile extends React.Component {
  ...
  componentDidMount() {
    fetch(`http://192.168.1.8:3000/users/verify`, {
      method: 'GET',
      headers: new Headers(
        authHeader())
    })
    ...
    return (
      <div className="container">
        ...
        <div className="user-info-cont">
          <h4 className="username_prg">{this.props.username}</h4>
          <p className="fullname_prg">{this.props.fullname}</p>
          <p className="email_prg">{this.props.email}</p>
        </div>
      </div>
    );
  }
