import React from 'react';
import './Profile.css'
import { handleResponse, authHeader } from '../_helpers';

export default class Profile extends React.Component {
  constructor(props) {
    super(props);
    this.props = props;

    this.state = {
      user_co: {
        corporateIdentityNumber: '',
        companyName: '',
        address: '',
        billingAddress: '',
        contactFirstName: '',
        contactLastName: '',
        companyEmail: ''
      },
      user_cli: {
        firstName: '',
        lastName: '',
        username: '',
        email: '',
        address: '',
        birthdate: ''
      }
    }
  }

  componentDidMount() {
    fetch(`${process.env.REACT_APP_BLINK}/company/validation`, {
      method: 'GET',
      headers: new Headers(
        authHeader())
    })
      .then(handleResponse)
      .then(result => {
        this.setState({ user: result })
      }).catch(err => console.log(err));
  }

  render() {
    return (
      <div className="container">
        <div className="row">
          <div className="col-md-4">
            <img className="prof-img" src="data:image/png;base64," alt="profileimage"/>
          </div>
        </div>
        {/* <div className="user-info-cont"> */}
        <div className="row">
          <div className="col-md-12">
            <div className="well">
              <h3 className="comp_prg">{this.state.user.companyName}</h3>
              <p className="fullname_prg">{this.state.user.contactFirstName + ' ' + this.state.user.contactLastName}</p>
              <p className="cin_prg">{this.state.user.corporateIdentityNumber}</p>
              <p className="email_prg">{this.state.user.companyEmail}</p>
              <p className="addr_prg">{this.state.user.address}</p>
              <p className="bill_addr_prg">{this.state.user.billingAddress}</p>
            </div>
          </div>
        </div>
      </div>
    );
  }
}