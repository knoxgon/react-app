...
export default class Profile extends React.Component {
  ...
  componentDidMount() {
    fetch(`${process.env.REACT_APP_BLINK}/client/validation`, {
      method: 'GET',
      headers: new Headers(
        authHeader())
    })
    ...
    return (
      <div className="container">
        ...
        <div className="user-info-cont">
          <h4 className="username_prg">{this.state.user.username}</h4>
          <p className="fullname_prg">{this.state.user.firstName + ' ' + this.state.user.lastName}</p>
          <p className="email_prg">{this.state.user.email}</p>
        </div>
      </div>
    );
  }
